...
**[下载 Windows 版](https://github.com/kennysu9598/moonlit-vigil/releases/tag/v0.6.0)** · **[参与开发](CONTRIBUTING.md)** · **[素材与许可](ASSET_LICENSES.md)**

![Windows 实战画面](docs/images/battle-phoenix.jpg)

## 直接玩

1. 打开上面的下载页，下载 `MoonlitVigil-v0.6.0-windows-x86_64.zip`。
...
- **18 招专属技能特效**：六单位 × 3 技能逐招设计（T1 快招 / T2 中技 / T3 大招 2.8 秒铺场），敌方招式带地面预警圈；护盾双通道可视化（血条盾格 + 角色描边光晕，破盾爆散）；Boss 蓄力三重预警（地面法阵 + 暗角聚焦 + 可打断读条）。
- **Kenney / OpenGameArt**：CC0 音效；**本项目 AI 音乐产线（ACE-Step，MIT 许可模型）**：战斗 / 胜利 / 败北三首 BGM；**edge-tts**：36 条中文角色配音；**Noto Sans SC**：OFL 字体。
...
这是 **v0.6.0 原型**，完成一场战斗，不是完整商业 RPG。本版把十八招专属特效、护盾双通道可视化与 Boss 蓄力三重预警铺满全场，并重制配音、战斗音效与战斗曲，新增标题曲。v0.5.0 补齐的完整声音链路（三曲制 BGM、6 单位 × 6 情境角色配音、总线级音频管理与静音）与战斗手感（出手预备、命中顿帧、受击闪白、伤害飘字）继续生效。两套灵兽主特效已通过本轮画面检查；其他未达到人物精细度的主特效保持关闭，技能规则仍可使用。凤凰只有四个关键姿态；人物也是有限姿态动画。Windows 包使用 Godot GUI 运行器 + PCK，可能显示 DEBUG 标题。Mac/Linux 尚未发行、未实测。

已做 100 种子战斗逻辑、100 种子自动策略测试、音频与配音 headless 逻辑测试（`test_audio` / `test_voice`）、局部动画检查，以及 Windows 实际输入从开场到胜利与重玩。配音已做机械响度对齐，真人听审仍待；详见 [测试说明](docs/TESTING.md)。
